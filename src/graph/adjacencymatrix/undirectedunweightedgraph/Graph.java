...
public class Graph<T> {
	...
	public T removeVertex(int index) {
		...
		boolean[] temp = adjacencyMatrix[index];
		...
		for (int i = 0; i < size() + 1; i++) {
			if (temp[i] == true) {
				if (removedVertexIndex < i)
					adjacencyMatrix[i - 1][removedVertexIndex] = false;
				else if (removedVertexIndex > i)
					adjacencyMatrix[i][removedVertexIndex] = false;
			}
		}
		for (int i = 0; i < size(); i++) {
